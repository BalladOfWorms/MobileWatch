#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Rev 291a — zone pass: The Garden of Ru'Hmet.
Zone string per zones.json = 'The Garden of RuHmet' (no apostrophe).
...
NM = [
    ("Ix'aern (DRK)",      "83", "Special (check ??? while Enveloped in sheer animosity)"),
    ("Ix'aern (DRG)",      SKIP, "Lottery (Aw'aern)"),
    ("Jailer of Faith",    SKIP, "Forced (trade HQ Euvhi Organ to ???)"),
    ("Jailer of Fortitude",SKIP, "Forced (trade 12 Ghrah M Chips to ???)"),
    ("Qn'zdei",            SKIP, "Timed (5 min)"),
    ("Ix'zdei",            SKIP, "Mission (Promathia 8-3)"),
]
...
ADV = [
    ...
    ("Aw'euvhi",         "78-80"),
    ("Aw'ghrah",         "79-81"),
    ("Aw'aern",          "79-83"),   # rule 2: four job rows, all 79-83
    ("Aw'zdei",          "80-83"),
]
...
# ---------------------------------------------------------------- apply
# NOT WRITTEN: Ix'aern (DRK) / (DRG). The file collapses all three wiki job-variants into ONE
# record `ix'aern` holding both zones — see the rev header. A split is a structural change.
WRITES_ZONE = [("aern's elemental", None)]     # page Lv reads ?-?
WRITES_LEVEL = [
    ("aw'aern", '81-84', '79-83'),
    ("aw'zdei", '76-80', '80-83'),
]
LV_EXTEND = [("aw'aern", [79, 84]), ("aw'zdei", [76, 83])]   # rule 9: union, never shrink
